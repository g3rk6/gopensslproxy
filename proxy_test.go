...
import (
	...
	"github.com/g3rk6/gopensslproxy/ext/image"
)
...
func TestSimpleHook(t *testing.T) {
	...
	proxy.OnRequest(gopensslproxy.SrcIpIs("127.0.0.1")).DoFunc(func(req *http.Request, ctx *gopensslproxy.ProxyCtx) (*http.Request, *http.Response) {
		req.URL.Path = "/bobo"
		return req, nil
	})
...
func TestReplaceReponseForUrl(t *testing.T) {
	proxy := gopensslproxy.NewProxyHttpServer()
	proxy.OnResponse(gopensslproxy.UrlIs("/koko")).DoFunc(func(resp *http.Response, ctx *gopensslproxy.ProxyCtx) *http.Response {
		resp.StatusCode = http.StatusOK
		resp.Body = ioutil.NopCloser(bytes.NewBufferString("chico"))
		return resp
	})
...
func (v VerifyNoProxyHeaders) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Header.Get("Connection") != "" || r.Header.Get("Proxy-Connection") != "" ||
		...
		v.Error("Got Connection header from gopensslproxy", r.Header)
	}
}
