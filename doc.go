/*
Package gopensslproxy provides a customizable HTTP proxy,
...
Adding a header to each request
	...
	proxy.OnRequest().DoFunc(func(r *http.Request,ctx *gopensslproxy.ProxyCtx) (*http.Request, *http.Response){
...
For printing the content type of all incoming responses
	...
	proxy.OnResponse().DoFunc(func(r *http.Response, ctx *gopensslproxy.ProxyCtx)*http.Response{
...
ReqCondition to the OnResponse function:
	...
	proxy.OnResponse(gopensslproxy.UrlIs("golang.org/pkg")).DoFunc(func(r *http.Response, ctx *gopensslproxy.ProxyCtx)*http.Response{
		println(ctx.Req.Host,"->",r.Header.Get("Content-Type"))
		return r
	})
...
Finally, we have convenience function to throw a quick response
	...
	proxy.OnResponse(hasGoProxyHeader).DoFunc(func(r*http.Response,ctx *gopensslproxy.ProxyCtx)*http.Response {
		r.Body.Close()
		return gopensslproxy.ForbiddenTextResponse(ctx.Req,"Can't see response with X-GoProxy header!")
	})
...
1. https://github.com/g3rk6/gopensslproxy/tree/master/examples/gopensslproxy-avgsize
...
3. https://github.com/g3rk6/gopensslproxy/blob/master/examples/gopensslproxy-httpdump/
...
4. https://github.com/g3rk6/gopensslproxy/tree/master/examples/gopensslproxy-no-reddit-at-worktime

Will allow browsing to reddit.com between 8:00am and 17:00pm

5. https://github.com/g3rk6/gopensslproxy/tree/master/examples/gopensslproxy-jquery-version

Will warn if multiple versions of jquery are used in the same domain.

6. https://github.com/g3rk6/gopensslproxy/blob/master/examples/gopensslproxy-upside-down-ternet/

Modifies image files in an HTTP response via gopensslproxy's image extension found in ext/.
