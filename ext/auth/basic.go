...
func Basic(realm string, f func(user, passwd string) bool) gopensslproxy.ReqHandler {
	return gopensslproxy.FuncReqHandler(func(req *http.Request, ctx *gopensslproxy.ProxyCtx) (*http.Request, *http.Response) {
		if !auth(req, f) {
...
//
// You probably want to use auth.ProxyBasic(proxy) to enable authentication for all proxy activities
func BasicConnect(realm string, f func(user, passwd string) bool) gopensslproxy.HttpsHandler {
	return gopensslproxy.FuncHttpsHandler(func(host string, ctx *gopensslproxy.ProxyCtx) (*gopensslproxy.ConnectAction, string) {
		if !auth(ctx.Req, f) {
			ctx.Resp = BasicUnauthorized(ctx.Req, realm)
			return gopensslproxy.RejectConnect, host
		}
		return gopensslproxy.OkConnect, host
	})
}
